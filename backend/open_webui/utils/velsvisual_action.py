...
class Action:
    ...
    @staticmethod
    def _run_generation(
        valves, entry: dict, prompt: str, sets: dict, image_urls: list[str], __event_emitter__
    ) -> tuple[dict, float]:
    ...
    async def action(
        ...
        if not valves or not valves.KIE_API_KEY.strip():
            return self._update_message(
                body, '⚠️ KIE_API_KEY не настроен. Попроси администратора задать ключ в настройках функции VelsVisual.'
            )
        ...
        try:
            ...
            if not parsed['prompt'] and not parsed['model']:
                return self._update_message(
                    body, 'Опиши, что сгенерировать, например: `кот в космосе --model google/nano-banana`'
                )
            ...
            if not parsed['sets'].pop('raw', False):
                ...
                if enhanced:
                    ...
                    self._emit(
                        __event_emitter__,
                        'Улучшение промпта недоступно или запрос вне бизнес-профиля — отправляю как есть',
                    )

            self._emit(__event_emitter__, f'Модель: {model_id} ({entry.get("category")})')
            st, elapsed = self._run_generation(
                valves, {**entry, 'id': model_id}, final_prompt, parsed['sets'], image_urls, __event_emitter__
            )
