...
SEED_MODELS = {
    ...
    'gpt-image/1.5-text-to-image': _m(
        'image',
        'jobs',
    ...
    'gpt-image/1.5-image-to-image': _m(
        'image',
        'jobs',
    ...
    'qwen/image-edit': _m(
        'image', 'jobs', image_field='image_url', description='Qwen image edit — редактирование изображения по промпту.'
    ),
    'flux-2/pro-text-to-image': _m(
        'image',
        'jobs',
    ...
    'recraft/remove-background': _m(
        'image',
        'jobs',
        prompt_field=None,
        image_field='image',
        required=['image'],
        description='Recraft — удаление фона. Промпт не нужен.',
    ),
    'flux-kontext-pro': _m(
        'image',
        'flux',
    ...
    'flux-kontext-max': _m(
        'image', 'flux', image_field='inputImage', description='FLUX Kontext Max — топовая версия Kontext.'
    ),
    'gpt4o-image': _m(
        'image',
        'gpt4o',
        image_field='filesUrl',
        image_list=True,
        required=['size'],
        defaults={'size': '1:1'},
    ...
    'veo3_fast': _m(
        'video', 'veo', image_field='imageUrls', image_list=True, description='Veo 3 Fast — быстрый и дешёвый.'
    ),
    'veo3_lite': _m('video', 'veo', image_field='imageUrls', image_list=True, description='Veo 3 Lite — облегчённый.'),
    'runway-gen3': _m(
        'video',
        'runway',
    ...
    'kling-2.6/text-to-video': _m(
        'video', 'jobs', description='Kling 2.6 text-to-video. Опции: sound (bool), aspect_ratio, duration ("5"|"10").'
    ),
    'kling-2.6/image-to-video': _m(
        'video',
        'jobs',
        image_field='image_urls',
        image_list=True,
    ...
    'kling/v2-5-turbo-text-to-video-pro': _m(
        'video', 'jobs', description='Kling 2.5 Turbo text-to-video pro. Опции: duration, aspect_ratio, cfg_scale.'
    ),
    'kling/v2-5-turbo-image-to-video-pro': _m(
        'video',
        'jobs',
        image_field='image_url',
        description='Kling 2.5 Turbo image-to-video pro. Опции: tail_image_url, duration, cfg_scale.',
    ),
    'hailuo/2-3-image-to-video-pro': _m(
        'video',
        'jobs',
        image_field='image_url',
    ...
    'hailuo/02-image-to-video-pro': _m(
        'video', 'jobs', image_field='image_url', description='Hailuo 02 image-to-video pro.'
    ),
    'bytedance/v1-pro-text-to-video': _m(
        'video', 'jobs', description='Seedance v1 Pro text-to-video. Опции: generate_audio, resolution, duration.'
    ),
    'wan/2-6-image-to-video': _m(
        'video',
        'jobs',
        image_field='image_urls',
        image_list=True,
...
def normalize_status(api: str, data: dict) -> dict:
    ...
    if api == 'flux':
        ...
        if flag == 1:
            return {
                'state': 'success',
                'urls': [data.get('resultImageUrl')] if data.get('resultImageUrl') else [],
                'fail_msg': None,
            }
        if flag in (2, 3):
            ...
            if resp.status_code == 200:
                ...
                if live_entries:
                    ...
                    MODELS_CACHE_PATH.write_text(
                        json.dumps({'fetchedAtMs': int(time.time() * 1000), 'entries': live_entries})
                    )
        except Exception:
            pass
