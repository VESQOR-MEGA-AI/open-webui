...
async def export_report(form_data: ExportForm, user=Depends(get_verified_user)):
    ...
    if (
        not brain_payload.get('reportId')
        and isinstance(brain_payload.get('body'), str)
        and brain_payload['body'].strip()
    ):
    ...
    if response.status_code < 300 and content:
        try:
            safe_title = (
                re.sub(r'[^A-Za-z0-9._-]+', '_', (form_data.title or form_data.reportId or 'report')).strip('_')
                or 'report'
            )
