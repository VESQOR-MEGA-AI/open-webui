		...
		<div class="min-h-0 flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin">
			...
			{:else}
				{#each items as item, idx (item.id)}
					<div
						class=" h-10 px-2 rounded-xl w-full text-left flex justify-between items-center text-sm font-normal hover:bg-gray-50/40 hover:text-gray-900 dark:hover:bg-gray-800/40 dark:hover:text-gray-100 {idx ===
						selectedIdx
						...
						<div class="pl-3 mb-1 flex flex-col gap-0.5">
							...
							{:else}
								{#each selectedFileItems as file, fileIdx (file.id)}
									<button
										class=" h-10 px-2 rounded-xl w-full text-left flex justify-between items-center text-sm font-normal hover:bg-gray-50/40 hover:text-gray-900 dark:hover:bg-gray-800/40 dark:hover:text-gray-100"
										type="button"
										on:click={() => {
											console.log(file);
											onSelect({
