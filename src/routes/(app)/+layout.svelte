<script lang="ts">
	...
	const runInit = async () => {
		...
		// VESQOR: upstream v0.11.1 added the toast below, but also an early `return`
		// on failure. We keep the toast and drop the return — init must always run to
		// completion (terminal, tool servers, shortcuts) rather than half-mount.
		try {
			await Promise.all([
